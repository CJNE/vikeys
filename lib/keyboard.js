...
Keyboard.prototype._init = function() {
  ...
  this.layout = [];
  rows = this.originalLayout.length;
  cols = this.originalLayout[0].length;
  ...
  this.board.on('keypress', function(ch, key) {
    ...
    switch(key.name) {
      ...
      case 'x': //Delete
        yank = [];
        yank.push(self.state.keys[self.state.currentKey].getMapping(layer));
      ...
      case 'p': //Yank
        ...
        yank.push(mapping);
        self.state.keys[self.state.currentKey].setMapping(layer, mapping);
      ...
      case 'space':
      case 'm':
        if(self.emit('select', self.state.keys[self.state.currentKey]))
