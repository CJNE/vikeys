...
exports.onceOn = function(event, clb) {
  if(typeof(onceListeners[event]) === 'undefined') onceListeners[event] = [];
  onceListeners[event].push(clb);
}
exports.clearOnce = function() {
  onceListeners = {};
}
...
exports.init = function(container, aKeyboard, globalState) {
  ...
  board.on('keypress', function(ch, key) {
    ...
    switch(key.name) {
      ...
      case 'space':
      case 'm':
        if(emit('select', state.keys[state.currentKey]))
          state.keys[state.currentKey].select();
        break;
      case 'i':
      case 'return':
        state.requestCommand(':set ', function(command) {
          state.debug("Got command "+command);
          return false;
        });
        return false;
    }
...
exports.eventListener = function(event, data) {
  ...
  var goX = 0, goY = 0;
  switch(data) {
    case 'focus': board.focus(); state.setHelp("Exit select mode with escape"); return false;
...
function emit(event, data) {
  var i, clb; 
  if(typeof(onceListeners[event]) !== 'undefined') {
    for(i = 0; i < onceListeners[event].length; i++) {
      clb = onceListeners[event][i];
      if(typeof(clb) === 'undefined' || clb === null) continue;
      state.debug("Calling once listener");
      if(!clb(event, data)) return false;
    }
    onceListeners[event] = [];
  }
