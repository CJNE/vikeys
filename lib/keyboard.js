  ...
  this.board.on('keypress', function(ch, key) {
    ...
    switch(key.name) {
      ...
      case 'x': //Delete
        ...
        self.state.redraw();
        self.emit("keyboard", "deleted");
        break;
      case 'y': //Yank
        self.yank = [];
        self.yank.push(self.state.keys[self.state.currentKey].getMapping(self.layer));
        self.emit("keyboard", "yanked");
        break;
      case 'p': //Paste
...
Keyboard.prototype.eventListener = function(event, data) {
  ...
  var goX = 0, goY = 0;
  switch(data) {
    case 'focus': this.board.focus(); this.container.screen.get('statusbar').setMessage("Exit select mode with escape"); return false;
    case 'layerdown': this.setLayer(this.layer - 1); this.container.screen.get('statusbar').setLayer(this.layer); return false;
    case 'layerup': this.setLayer(this.layer + 1);  this.container.screen.get('statusbar').setLayer(this.layer); return false;
...
Keyboard.prototype.go = function(goX, goY) {
  ...
  this.state.redraw();
  this.container.screen.get('statusbar').setMessage("Key "+goToKey); //+" mapped to: "+keyInfo
}
