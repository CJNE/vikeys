...
Keybox.prototype.__proto__ = blessed.Textbox.prototype;
Keybox.prototype.type = 'keybox';
Keybox.prototype._listener = function(ch, key) {
  ...
  if (key.name === 'enter' || key.name === 'tab') {
    this._done(null, this.isValid(this.value) ? this.value : null);
    return;
  }
  ...
  if (this.value !== value) {
    if(this.isValid(this.value)) this.emit('change', this.value);
    this.screen.render();
  }
  return false;
};
Keybox.prototype.isValid = function(value) {
  return true;
}

Keybox.prototype.doComplete = function(input) {
  return '';
}
/**
 * Number input box
...
Listmenu.prototype.render = function() {
  ...
  this.items.forEach(function(el, i) {
    if (i < self.topBase || i >= self.height - self.iheight + self.topBase) {
      el.hide();
    } else {
      el.rtop = drawn;
      drawn += 1; //el.height;// + 2;
...
Listmenu.prototype.select = function(offset) {
  ...
  this.screen.debug("Top offset: "+this.topOffset);
  this.emit('change', this.commands[this.selected], this.selected);
...
exports.Keybox = exports.keybox = Keybox;
