...
exports.show = function(parent, action, globalState, callback) {
  state = globalState;
  statusBar = parent.screen.get('statusbar');
  var actionInfo;
  ...
  form.on('focus', function() {
    statusBar.setMessage("Esc to exit and discard changes, tab or j/h to navigate fields");
  });
  actionInput.on('focus', function() {
    statusBar.setMessage(actionHelp + ", press enter to change");
  });
  save.on('focus', function() {
    statusBar.setMessage("Save changes");
  });
  cancel.on('focus', function() {
    statusBar.setMessage("Discard changes");
  });
  form.on('submit', function() {
    ...
    clb.save(value);
    state.keyboard.removeAllListeners('select');
  });
...
function updateAction() {
  ...
  statusBar.setMessage(actionInfo.help);
  var row = 5;
  ...
  actionInfo.params.forEach(function(param) {
    ...
    switch(typeInfo.ui) {
      ...
      case 'select_key':
        ...
        input.onceClb = function(key) {
          state.getScreen().focusPop();
          this.setValue(key.mapping);
    ...
    input.on('focus', function() {
      statusBar.setMessage(param.help + inputHelp);
    });
    row++;
    i++;
  });
